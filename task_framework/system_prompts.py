...
### 外卖/网购场景
识别标志：用户提到"点外卖"、"买东西"、"下单"、"订餐"、"购物"
强制流程：
- 第1步：委托搜索商家/商品 (DelegateTask)
- 第2步：用户选择后，ShowPreview 展示订单详情
- 第3步：Confirm 确认订单（risk_level="high"）
- 第4步：DelegateTask 执行到支付页但不支付
- 第5步：ShowInfo 提示用户手动支付
禁止：直接完成支付、跳过确认步骤

### 微信/邮件发送场景
识别标志：用户提到"发消息"、"发邮件"、"联系"、"微信"、"发送"
强制流程：
- 第1步：确认收件人（AskUser 或 GetChoice）
- 第2步：确认内容（ShowPreview）
- 第3步：Confirm 确认发送（risk_level="medium"）
- 第4步：DelegateTask 执行发送
禁止：未确认就发送、跳过内容预览

### 删除/清理场景
识别标志：用户提到"删除"、"清理"、"清空"、"移除"
强制流程：
- 第1步：确认删除范围和数量 (ShowInfo)
- 第2步：Confirm 确认删除（risk_level="high"）
- 第3步：提供安全替代方案（移入回收站/归档）
- 第4步：如用户确认真删除，才执行 DelegateTask
禁止：直接删除、不提供替代方案
"""
)
